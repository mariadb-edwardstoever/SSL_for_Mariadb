...
if [ "$LINUX_FAMILY" == "REDHAT" ]; then OPTIONS_FILE='/etc/my.cnf.d/client.cnf'; fi
...
OUTPUT="Note that the mariadb client for 11.4 and higher does not require that you configure
certificates. They are auto-generated by the software.

There are two methods for connecting the mariadb client to a server using SSL.
