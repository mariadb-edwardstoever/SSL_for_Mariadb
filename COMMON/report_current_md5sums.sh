...
CURRENT="$(cat ${OWNORGDET} | tr -d "[:space:]" | md5sum |  cut -d' ' -f1)"
...
CURRENT="$(cat ${OWNEXTENS} | tr -d "[:space:]" | md5sum |  cut -d' ' -f1)"
...
CURRENT="$(cat ${CACONFIG} | tr -d "[:space:]" | md5sum |  cut -d' ' -f1)"
...
CURRENT="$(cat ${VARSSH} | tr -d "[:space:]" | md5sum |  cut -d' ' -f1)"
